...
def train(epo_num=50, show_vgg_params=False):
    ...
    for epo in range(epo_num):
        ...
        for index, (bag, bagmsk) in enumerate(train_dataloader):
            ...
            output_np = output.cpu().detach().numpy().copy()        #out_put_np.shape = (4,2,160,160)
            ...
            if np.mod(index, 15) == 0:
                ...
                vis.images(bag_mask_np[:,None,:,:], win='train_label', opts=dict(title='label'))
        ...
        with torch.no_grad():
            for index, (bag, bagmsk) in  enumerate(test_dataloader):
                ...
                if np.mod(index, 15) == 0:
                    ...
                    vis.images(bag_mask_np[:, None, :, :], win='test_label', opts=dict(title='label'))
